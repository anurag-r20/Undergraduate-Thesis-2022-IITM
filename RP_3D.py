...
# Parameters
Dring = 1 
Lx, Ly, Lz = 4*Dring, 4*Dring, 10*Dring # size of confinement
Nx, Ny, Nz = 64, 64, 128 # grid size
dealias = 3/2
dtype = np.float64
sigma_0 = 0.04
R_0 = 0.5
...
omega = dist.VectorField(coords, name='omega', bases=(xbasis, ybasis, zbasis)) # vorticity field
...
f = dist.VectorField(coords, name='f', bases=(xbasis,ybasis,zbasis)) # initial vorticity field
xcen = Lx*0.5
ycen = Ly*0.5
zcen = Lz*0.25
...
problem.add_equation("dt(u) - nu*div(grad_u) + grad(p) - T*ez = - u@grad(u)") # 
...
try:
    ...
    while solver.proceed:
        ...
        if solver.sim_time != stop_sim_time:
            ...
            file1.write(' Variables="X","Y","Z","u","v","w","wx","wy","wz","T"\n') #  
            file1.write(' Zone  I=64, J=64, K=128, F=POINT\n')
            for k in range(Nz):
            	for j in range(Ny):
                    for i in range(Nx):
                        file1.write(f" {X[i]}\t{Y[j]}\t{Z[k]}\t{ux[i,j,k]}\t{uy[i,j,k]}\t{uz[i,j,k]}\t{wx[i,j,k]}\t{wy[i,j,k]}\t{wz[i,j,k]}\t{T['g'][i,j,k]}\n") # 
            file1.close()
